...
public class PhysicsUtils {
    ...
    public static void intersect(Ball a, Ball b) {
        ...
        if (distSquared <= (a.radius + b.radius) * (a.radius + b.radius)) {
            ...
            // one another.
            boolean collisionDetected = dotProduct > 0;
            // System.out.println("xDist=" + xDist + ", yDist=" + yDist + ", distSquared=" + distSquared +
            // ", dotProduct="
            // + dotProduct + ", collisionDetected=" + collisionDetected);
            ...
            float distXX = (a.x - b.x) * (a.x - b.x);
            float distYY = (a.y - b.y) * (a.y - b.y);
            float calculatedDistance = (float) Math.sqrt(distXX + distYY);
            float distance = a.radius + b.radius;
            boolean collision = calculatedDistance < distance;
            System.out.println("calculatedDistance=" + calculatedDistance + ", distance=" + distance + ", collision="
                    + collision);
            if (collision) {
                a.increaseCollisionCount();
                b.increaseCollisionCount();
            }
        }
    }
